...
export default function ProjectCard({ title, description, tags, href, className }: ProjectCardProps) {
  const content = (
    <div className={cn(
      ...
      {tags && tags.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((t) => (
            <span key={t} className="inline-flex items-center rounded-md bg-accent px-2 py-1 text-xs font-medium text-accent-foreground border border-border/60">
              {t}
            </span>
          ))}
        </div>
